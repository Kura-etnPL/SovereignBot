...
if (!existsSync(electronExe)) {
  console.error("[verify-p37] local Electron binary is missing; refusing to install or download dependencies");
  process.exit(2);
}
const tempRoot = mkdtempSync(join(tmpdir(), "sovereign-p37-"));
const evidenceDir = process.env.SOVEREIGNBOT_PRODUCT_EVIDENCE_DIR ?? join(desktopRoot, "..", "docs", "acceptance");
mkdirSync(evidenceDir, { recursive: true });
const child = spawn(electronExe, ["--disable-gpu", "--user-data-dir=" + join(tempRoot, "electron-user-data"), "src/main/verify-p37-settings-entry.js"], {
  cwd: desktopRoot,
...
const timer = setTimeout(() => {
  if (done) return;
  console.error("[verify-p37] timed out; terminating Electron");
...
child.on("exit", (code, signal) => {
  ...
  clearTimeout(timer);
  cleanup();
  console.error(`[verify-p37] Electron exited code=${code} signal=${signal}`);
...
child.on("error", (error) => {
  ...
  clearTimeout(timer);
  cleanup();
  console.error(`[verify-p37] spawn failed: ${String(error?.stack ?? error)}`);
  process.exit(1);
});
