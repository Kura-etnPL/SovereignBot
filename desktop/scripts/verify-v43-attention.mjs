...
if (!existsSync(join(DESKTOP_ROOT, "node_modules", "electron", "package.json"))) {
  console.error("[verify-v43] electron not installed (run `npm ci` in desktop/)");
  process.exit(2);
}

console.error(`[verify-v43] spawning isolated Attention gate (timeout ${TIMEOUT_MS / 1000}s)`);
const child = spawn(electronBin, ["src/main/verify-v43-attention-entry.js"], {
  cwd: DESKTOP_ROOT,
...
const timer = setTimeout(() => {
  if (done) return;
  console.error("[verify-v43] timed out; terminating Electron");
...
child.on("exit", (code, signal) => {
  ...
  clearTimeout(timer);
  console.error(`[verify-v43] Electron exited code=${code} signal=${signal}`);
  if ((code ?? 1) !== 0) {
...
child.on("error", (error) => {
  ...
  clearTimeout(timer);
  console.error(`[verify-v43] spawn failed: ${String(error?.stack ?? error)}`);
  process.exit(1);
});
