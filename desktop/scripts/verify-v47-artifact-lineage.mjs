...
if (!existsSync(electronExe)) {
  console.error("[verify-v47] local Electron binary is missing; refusing to install or download dependencies");
  process.exit(2);
}

const tempRoot = mkdtempSync(join(tmpdir(), "sovereign-v47-"));
mkdirSync(join(tempRoot, "electron-user-data"), { recursive: true });
console.error(`[verify-v47] spawning hidden artifact lineage gate (timeout ${TIMEOUT_MS / 1000}s)`);
...
const timer = setTimeout(() => {
  if (done) return;
  console.error("[verify-v47] timed out; terminating Electron");
...
child.on("exit", (code, signal) => {
  ...
  clearTimeout(timer);
  cleanup();
  console.error(`[verify-v47] Electron exited code=${code} signal=${signal}`);
  if ((code ?? 1) !== 0) {
...
child.on("error", (error) => {
  ...
  clearTimeout(timer);
  cleanup();
  console.error(`[verify-v47] spawn failed: ${String(error?.stack ?? error)}`);
  process.exit(1);
});
