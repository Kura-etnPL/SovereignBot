...
export const V3_IPC_CHANNELS = Object.freeze({
    ...
    "conversation:get": spec(1024, (payload) => { const value = objectPayload(payload); exact(value, new Set(["conversationId", "limit", "beforeMessageId", "aroundMessageId"])); if (value.beforeMessageId !== undefined && value.aroundMessageId !== undefined) throw new Error("conversation page cursor is ambiguous"); return { conversationId: identifier(value.conversationId, "conversationId"), limit: value.limit === undefined ? 100 : positiveInteger(value.limit, "limit", 1, 100), ...(value.beforeMessageId === undefined ? {} : { beforeMessageId: messageIdentifier(value.beforeMessageId, "beforeMessageId") }), ...(value.aroundMessageId === undefined ? {} : { aroundMessageId: messageIdentifier(value.aroundMessageId, "aroundMessageId") }) }; }),
