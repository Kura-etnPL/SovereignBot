...
export async function runVerifyP11TeamPacks({ app }) {
    ...
    try {
        ...
        const firstPartyButtons = await invoke(win, "async()=>[...document.querySelectorAll('#product-packs-page .settings-card button')].map((button)=>button.textContent)");
        check("first-party recipes remain read-only in the gallery", !firstPartyButtons.some((label) => label.includes("Edit recipe")), JSON.stringify(firstPartyButtons));
        ...
        await invoke(win, `async()=>{const card=document.querySelector('[data-team-pack-id="${duplicateRecipeId}"]'); [...(card?.querySelectorAll('button')||[])].find((button)=>button.textContent.includes('Edit recipe'))?.click(); return true}`);
        await waitFor("structured Team Pack editor", async () => await invoke(win, "async()=>Boolean(document.getElementById('team-pack-editor-dialog')?.open)"));
        const editorBefore = await invoke(win, "async()=>({name:document.getElementById('team-pack-editor-name')?.value, description:document.getElementById('team-pack-editor-description')?.value, coworkers:document.querySelectorAll('#team-pack-editor-coworkers .team-pack-editor-row').length, channels:document.querySelectorAll('#team-pack-editor-channels .team-pack-editor-row').length, playbooks:document.querySelectorAll('#team-pack-editor-playbooks .team-pack-editor-row').length, rawJson:document.querySelector('#team-pack-editor-dialog textarea[id*=json]') !== null})");
        check("custom Edit opens the structured declarative recipe editor", editorBefore.coworkers >= 2 && editorBefore.channels >= 1 && editorBefore.playbooks >= 1 && !editorBefore.rawJson, JSON.stringify(editorBefore));
        const editedByForm = await invoke(win, "async()=>{const dialog=document.getElementById('team-pack-editor-dialog'); const set=(selector,value)=>{const field=dialog.querySelector(selector); if(field){field.value=value; field.dispatchEvent(new Event('input',{bubbles:true}));}}; set('#team-pack-editor-name','P21 Edited Recipe'); set('#team-pack-editor-description','P21 structured editor recipe'); set('.team-pack-editor-coworker-name','P21 Lead'); set('.team-pack-editor-coworker-role','P21 Coordinator'); set('.team-pack-editor-coworker-profile','deep'); set('.team-pack-editor-coworker-instructions','Coordinate the safe recipe.'); set('.team-pack-editor-channel-name','P21 Channel'); set('.team-pack-editor-channel-kind','work'); set('.team-pack-editor-channel-instructions','Keep this work channel focused.'); set('.team-pack-editor-playbook-name','P21 Method'); set('.team-pack-editor-playbook-description','P21 method description'); set('.team-pack-editor-playbook-output','P21 bounded output'); return true}");
        check("structured editor accepts full composition fields", editedByForm === true, JSON.stringify(editedByForm));
        await invoke(win, "async()=>document.getElementById('team-pack-editor-form')?.requestSubmit()");
        await waitFor("structured editor save", async () => await invoke(win, "async()=>!document.getElementById('team-pack-editor-dialog')?.open"));
        await invoke(win, "async()=>{const input=document.getElementById('team-pack-search-page'); input.value='P21 Edited Recipe'; input.dispatchEvent(new Event('input',{bubbles:true})); return true}");
        await waitFor("custom Team Pack edit", async () => (await invoke(win, "async()=>[...document.querySelectorAll('#product-packs-page .settings-card h3')].map((node)=>node.textContent)" )).includes("P21 Edited Recipe"));
        ...
        const editedRecipe = await invoke(win, `async()=>window.sovereignbot.teams.exportPackRecipe({packId:${JSON.stringify(duplicateRecipeId)}})`);
        check("structured editor saves the complete declarative composition through preload IPC", editedCards.some((card) => card.includes("P21 Edited Recipe")) && editedRecipe?.description === "P21 structured editor recipe" && editedRecipe.coworkers?.some((entry) => entry.name === "P21 Lead" && entry.modelBinding?.profile === "deep") && editedRecipe.channels?.some((entry) => entry.name === "P21 Channel" && entry.kind === "work" && entry.instructions === "Keep this work channel focused.") && editedRecipe.playbooks?.some((entry) => entry.name === "P21 Method" && entry.description === "P21 method description" && entry.expectedOutput === "P21 bounded output"), JSON.stringify({ cards: editedCards, recipe: editedRecipe }));
        await invoke(win, `async()=>{const card=document.querySelector('[data-team-pack-id="${duplicateRecipeId}"]'); [...(card?.querySelectorAll('button')||[])].find((button)=>button.textContent.includes('Edit recipe'))?.click(); return true}`);
        await waitFor("reopened structured editor", async () => await invoke(win, "async()=>Boolean(document.getElementById('team-pack-editor-dialog')?.open)"));
        await invoke(win, "async()=>{const field=document.getElementById('team-pack-editor-name'); field.value='P21 Cancelled Change'; field.dispatchEvent(new Event('input',{bubbles:true})); [...document.querySelectorAll('#team-pack-editor-dialog button')].find((button)=>button.textContent.includes('Cancel'))?.click(); return true}");
        await waitFor("structured editor cancel", async () => await invoke(win, "async()=>!document.getElementById('team-pack-editor-dialog')?.open"));
        ...
        check("structured editor cancel performs no write", afterCancel?.name === "P21 Edited Recipe", JSON.stringify(afterCancel));
