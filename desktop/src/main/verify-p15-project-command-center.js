...
const EVIDENCE_DIR = join(WORKTREE_ROOT, "_evidence_v54_2026-09-03");
...
function handlers(fixture) {
  ...
  return {
    ...
    "conversation:list": () => conversationStore.list(), "conversation:get": ({ conversationId }) => conversationStore.get(conversationId),
    ...
    "search:query": (payload) => search.query(payload), "palette:list": () => ({ commands: [] }), "palette:execute": () => ({ ok: true }), "job:list": () => jobs.listJobs(), "job:attention": (payload) => jobs.attentionJobs(payload), "thisPc:list": () => ({ items: [] }), "data:status": () => ({ backups: [] }), "data:listBackups": () => ({ backups: [] }), "update:status": () => ({ available: false }),
  };
}
...
export async function runVerifyP15ProjectCommandCenter({ app }) {
  ...
  let dataDir; let fixture; let win; let unbind; let uninstallProtocol; let fatal; let projectId; let teamId; let conversationId; let artifactId; let memoryId; let routineId; let triggerId;
  try {
