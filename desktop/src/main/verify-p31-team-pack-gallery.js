...
export async function runVerifyP31TeamPackGallery({ app }) {
  ...
  try {
    ...
    await waitFor("dedicated page finds edited custom recipe", async () => await invokeChecked(win, "dedicated page recipe found", `async()=>Boolean(document.querySelector('#product-packs-page [data-team-pack-id="${duplicateRecipeId}"]'))`));
    const pageEntry = await invokeChecked(win, "dedicated page card snapshot", `async()=>{const card=document.querySelector('#product-packs-page [data-team-pack-id="${duplicateRecipeId}"]'); return {title:card?.querySelector('h3')?.textContent,buttons:[...(card?.querySelectorAll('button')||[])].map((button)=>button.textContent)}}`);
    ...
    await invokeChecked(win, "dedicated page open editor", `async()=>document.querySelector('#product-packs-page [data-team-pack-id="${duplicateRecipeId}"] button:last-child')?.click()`);
