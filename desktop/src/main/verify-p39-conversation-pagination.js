...
export async function runVerifyP39ConversationPagination({ app } = {}) {
  ...
  try {
    ...
    await invoke(win, `async()=>{document.getElementById("conversation-load-older")?.click(); return true}`);
    await waitFor(async () => await invoke(win, `async()=>document.querySelectorAll("#conversation-messages [data-message-id]").length===300`), "second older message page");
    await invoke(win, `async()=>{document.getElementById("conversation-load-older")?.click(); return true}`);
    await waitFor(async () => await invoke(win, `async()=>document.querySelector("#conversation-messages [data-message-id] .chat-text")?.textContent==="P39 message 50"`), "sliding historical window");
    await sleep(100);
    ...
    check("Load older preserves the scroll anchor across 3 pages", Math.abs(anchorDelta) < 8, safeJson({ before: anchorBefore, after: { top: olderDom.top, height: olderDom.height }, anchorDelta: Number(anchorDelta.toFixed(2)) }));
