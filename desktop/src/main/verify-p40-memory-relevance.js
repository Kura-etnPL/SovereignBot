...
export async function runVerifyP40MemoryRelevance({ app } = {}) {
  ...
  try {
    ...
    check("exact title and phrase ranking stay scope-bound", exactResult.resultCount === 1 && exactResult.memories?.[0]?.id === exact.id && exactResult.memories?.[0]?.matchReason?.key === "title-exact", safeJson({ count: exactResult.resultCount, id: exactResult.memories?.[0]?.id, reason: exactResult.memories?.[0]?.matchReason }));
    ...
    await waitFor(async () => await invoke(win, `async()=>document.querySelectorAll("#memory-list .memory-row").length===1`), "Memory UI tag results");
