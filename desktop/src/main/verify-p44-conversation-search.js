...
export async function runVerifyP44ConversationSearch({ app } = {}) {
    ...
    try {
        ...
        await waitFor(async () => (await invoke(win, `async()=>{const targetId=${JSON.stringify(targetMessage.id)}; return !!document.querySelector('[data-message-id="'+targetId+'"]')}`)), "anchored historical message in the UI");
