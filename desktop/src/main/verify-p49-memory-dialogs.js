...
export async function runVerifyP49MemoryDialogs({ app }) {
  ...
  try {
    ...
    await waitForRenderer(win, `async()=>document.querySelector("#view-memory:not(.hidden)") !== null`, "main Memory view");
    await waitForRenderer(win, `async()=>Boolean([...document.querySelectorAll("#memory-owner option")].some((option)=>option.value === ${targetId}))`, "target Memory owner option");
    await invoke(win, `async()=>{const scope=document.getElementById("memory-scope"); if(scope && scope.value !== "coworker"){scope.value="coworker"; scope.dispatchEvent(new Event("change",{bubbles:true}));} const owner=document.getElementById("memory-owner"); if(owner){owner.value=${targetId}; owner.dispatchEvent(new Event("change",{bubbles:true}));} return {scope:scope?.value,owner:owner?.value}}`);
    await waitForRenderer(win, `async()=>Boolean(document.querySelector("#view-memory:not(.hidden)") && document.getElementById("memory-scope")?.value === "coworker" && document.getElementById("memory-owner")?.value === ${targetId} && document.querySelector("#memory-list .memory-row")?.textContent.includes("P49 Memory") && document.querySelector("#memory-list .memory-row")?.textContent.includes("Before edit"))`, "target main Memory row");
    await invoke(win, `async()=>{const rows=[...document.querySelectorAll("#memory-list .memory-row")]; const card=rows.find((entry)=>entry.textContent.includes("P49 Memory") && entry.textContent.includes("Before edit")); const button=[...(card?.querySelectorAll("button")||[])].find((entry)=>entry.textContent.includes("Edit")); button?.click(); return Boolean(button)}`);
    await waitForRenderer(win, `async()=>document.getElementById("memory-edit-dialog")?.open===true`, "main Memory Edit dialog");
    const mainEdit = await invoke(win, `async()=>({open:!!document.getElementById("memory-edit-dialog")?.open,title:document.getElementById("memory-edit-title")?.value,content:document.getElementById("memory-edit-content")?.value})`);
