...
test("P50 hidden gate covers card/detail actions, retry counts, isolation, and deterministic evidence", () => {
  for (const expression of [
    ...
    /Work Retry reaches completed state through the existing Job authority/,
    /Work Dismiss clearly clears Attention without affecting another Job/,
    /Job Details Approve reaches completed state through the shared pending path/,
    /Job Details Pause is available only for a legal queued nonterminal state/,
    /Job Details Resume reaches completed state from a legal waiting state/,
    /Job Details Dismiss clears Attention with scoped feedback/,
    /P50 Resume fixture is waiting before its detail action/,
    /const pauseJob = jobs\.submitJob\(/,
    /const resumeJob = jobs\.submitJob\(/,
    ...
    /P50 Pause fixture remains queued before its detail action/,
    /P50 detail diagnostics/,
    /openClicked/,
    /dialogOpen/,
    /buttonClassName/,
    /feedback:document\.getElementById\("job-detail-feedback"\)/,
    /const workerNodeId = "worker_[0-9a-f]{16}"/,
    /import \{ coworkerAgentId \} from "\.\/provider-roster\.js"/,
    /agentId: coworkerAgentId\(entry\.id\)/,
