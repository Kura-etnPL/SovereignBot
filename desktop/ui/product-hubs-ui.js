...
(() => {
  ...
  function renderPlaybooks(items, teams) {
    ...
    for (const item of items) {
      ...
      if (teams.length) actions.append(teamSelect, button("Assign / 分配", async () => { await api.playbooks.assign({ playbookId: item.id, teamId: teamSelect.value }); await refresh(); }));
  ...
  function setup() { $("nav-product-hubs")?.addEventListener("click", async () => { switchView("product-hubs"); try { await refresh(); } catch (e) { error($("product-playbooks"), e); } }); $("product-hubs-refresh")?.addEventListener("click", () => void refresh()); $("playbook-create")?.addEventListener("click", () => void createPlaybook()); }
