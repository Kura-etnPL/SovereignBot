...
(() => {
  ...
  async function refresh() {
    ...
    mode = "search"; const response = await api.search.query({ query: value, types: [...searchTypes], ...(projectId ? { projectId } : {}), status, limit: 50 }); if (sequence !== refreshSequence) return; for (const item of response.results ?? []) { const reason = MATCH_REASON_LABELS.get(item.matchReason?.key) ?? "Match / 匹配"; addResult(item.title, `${item.type} · ${item.subtitle} · ${item.status} · ${reason}`, () => navigate(item)); } if (!response.results?.length) { results.append(make("p", "setting-feedback", "No matching visible results / 没有匹配的可见结果")); setStatus(""); } else { setStatus(`${response.total ?? response.results.length} result${(response.total ?? response.results.length) === 1 ? "" : "s"}${response.hasMore ? " · refine filters to see more / 可继续缩小筛选" : ""}`); } paintSelection();
  }
