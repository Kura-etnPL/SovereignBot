import { chatGPTWebAdapterFor, registerAgentChatGPTWebAdapter } from "./chatgpt-web-registry.js";

const MAX_TEXT = 20_000;
const MAX_CONTINUATION = 512;
const PRIVATE_RESULT_KEYS = new Set(["sessionId", "conversationId", "profileDir", "storageState", "cookies", "continuationUrl"]);

export { registerAgentChatGPTWebAdapter };

function safeText(value, label) {
    if (typeof value !== "string" || !value.trim() || value.length > MAX_TEXT)
        throw new Error(`ChatGPT Web ${label} is invalid`);
    return value;
}
...
function safeContinuation(value) {
    if (typeof value !== "string" || !value || value.length > MAX_CONTINUATION || !/^[A-Za-z0-9][A-Za-z0-9._:-]*$/.test(value))
        throw new Error("ChatGPT Web continuation is invalid");
    return value;
}
...
function requestForTask(task, agent) {
    ...
    if (!input || typeof input !== "object" || Array.isArray(input))
        throw new Error("ChatGPT Web task input must be an object");
    return {
        title: safeText(String(task.title ?? "ChatGPT Web task"), "task title").slice(0, 240),
        instruction: safeInstruction(String(input.instruction ?? "")),
        conversation: safeConversation(input.conversation),
        model: typeof agent.harness?.model === "string" && agent.harness.model ? agent.harness.model : "sol",
    };
}

function safeOutput(result) {
    if (!result || typeof result !== "object" || Array.isArray(result))
        throw new Error("ChatGPT Web adapter returned an invalid result");
    for (const key of PRIVATE_RESULT_KEYS) {
        if (Object.hasOwn(result, key))
            throw new Error(`ChatGPT Web adapter leaked private result field: ${key}`);
    }
    const text = result.text ?? result.output?.text;
    if (typeof text !== "string")
        throw new Error("ChatGPT Web adapter returned no text");
...
export class ChatGPTWebHarness {
    constructor(agent) {
        this.agent = agent;
    }

    async run(context) {
        const adapter = chatGPTWebAdapterFor(this.agent);
        if (!adapter)
            throw new Error("ChatGPT Web adapter is not registered");
        const request = requestForTask(context.task, this.agent);
        const prior = context.task.harnessState?.kind === "chatgpt-web"
            ? safeContinuation(context.task.harnessState.continuationRef)
            : undefined;
        ...
        try {
            if (context.signal.aborted) {
                await cancel();
                return { ok: false, error: "ChatGPT Web task cancelled" };
            }
            ...
            const aborted = new Promise((_, reject) => {
                if (context.signal.aborted)
                    reject(new Error("ChatGPT Web task cancelled"));
                else
                    context.signal.addEventListener("abort", () => reject(new Error("ChatGPT Web task cancelled")), { once: true });
            });
            const result = await Promise.race([operation, aborted]);
            if (cancelRequested || context.signal.aborted)
                return { ok: false, error: "ChatGPT Web task cancelled" };
            const normalized = safeOutput(result);
            if (normalized.continuationRef)
                await context.updateHarnessState?.({ kind: "chatgpt-web", continuationRef: normalized.continuationRef });
