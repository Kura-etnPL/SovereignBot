...
export async function handleOperatorApiRequest(runtime,request,response,url){
    ...
    try{
        ...
        if(request.method==="GET"&&url.pathname==="/operator/audit"){const limit=Math.max(1,Math.min(500,Number(url.searchParams.get("limit")??100)));const rows=await runtime.audit.readAll();const tasks=await runtime.orchestrator.listTasks();send(response,200,publicRuntimeRecords(rows.slice(-limit).reverse(),tasks));return}
        ...
        if(request.method==="GET"&&p[0]==="operator"&&p[1]==="tasks"&&p[2]&&p[3]==="events"){const events=await runtime.orchestrator.listTaskEvents(p[2]);const tasks=await runtime.orchestrator.listTasks();send(response,200,publicRuntimeRecords(events,tasks));return}
