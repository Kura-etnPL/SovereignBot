...
export async function handleOperatorApiRequest(runtime,request,response,url){
    ...
    try{
        ...
        if(request.method==="GET"&&url.pathname==="/operator/policy"){send(response,200,{active:validatePolicyDraft(runtime.config.policy),editable:false,note:"Snapshot only. Editing or dry-running this copy does not change the active runtime policy."});return}
