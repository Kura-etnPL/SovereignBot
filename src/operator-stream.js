...
export async function handleOperatorStream(runtime, request, response) {
    ...
    if (!loopbackHost(runtime.config.bindHost ?? "127.0.0.1") || !loopbackRemote(request.socket.remoteAddress)) {
        sendJson(response, 403, { error: "operator telemetry is available only on loopback" });
        return;
    }
    if (!originAllowed(request)) {
        sendJson(response, 403, { error: "cross-origin operator telemetry refused" });
        return;
    }

    const sessionToken = bearer(request);
    if (!await runtime.operatorSessions.authenticate(sessionToken)) {
        sendJson(response, 401, { error: "invalid or expired operator session" });
        return;
    }
    ...
    heartbeatTimer = setInterval(() => {
        emit({ source: "system", type: "heartbeat", at: new Date().toISOString() });
    }, OPERATOR_STREAM_HEARTBEAT_MS);
    heartbeatTimer.unref?.();

    sessionTimer = setInterval(async () => {
        if (closed || checkingSession)
            return;
        checkingSession = true;
        try {
            const valid = await runtime.operatorSessions.authenticate(sessionToken);
            if (!valid && !closed) {
    ...
    }, OPERATOR_STREAM_SESSION_CHECK_MS);
    sessionTimer.unref?.();
}
