...
test("worker telemetry is passive and reports true in-flight harness execution", async () => {
    ...
    try {
        const queued = await runtime.orchestrator.submit({
            title: "delayed work",
            requiredCapabilities: ["demo"],
            preferredAgentId: "worker",
        });
        ...
        await waitFor(async () => {
            ...
            return snapshot.find((entry) => entry.id === "worker")?.inFlightHarnessCount === 1;
        });

        const during = await collectWorkerTelemetry(runtime.orchestrator);
        const workerDuring = during.find((entry) => entry.id === "worker");
        ...
        const after = await collectWorkerTelemetry(runtime.orchestrator);
        assert.equal(after.find((entry) => entry.id === "worker").inFlightHarnessCount, 0);
    }
    finally {
        await runtime.close();
    }
...
test("worker telemetry never exposes provider session ids or task payloads", async () => {
    ...
    try {
        ...
        await runtime.orchestrator.tasks.update(task.id, (current) => ({
            ...current,
            assignedAgentId: "worker",
            harnessState: { kind: "codex", sessionId: secretSession },
        }));
...
test("supervisor is not counted as compatible execution capacity unless task explicitly allows it", async () => {
    ...
    try {
        ...
        await runtime.orchestrator.submit({
            title: "supervisor allowed",
            requiredCapabilities: ["demo"],
            allowSupervisorExecution: true,
        });
        const snapshot = await collectWorkerTelemetry(runtime.orchestrator);
        const worker = snapshot.find((entry) => entry.id === "worker");
        const supervisor = snapshot.find((entry) => entry.id === "supervisor");
        assert.equal(worker.compatibleQueuedCount, 2);
        assert.equal(supervisor.compatibleQueuedCount, 1);
    }
    finally {
        await runtime.close();
    }
...
test("operator workers endpoint remains session-authenticated and redacted", async () => {
    ...
    try {
        ...
        const response = await fetch(`${server.url}/operator/workers`, {
